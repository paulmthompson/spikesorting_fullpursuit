...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        raise ValueError(
            "Must input both npy voltage array and ground truth pickle file or use default (no inputs)."
        )
    elif len(sys.argv) == 3:
        ...
        raise ValueError(
            "Requires 2 inputs, voltage file and ground truth file, or zero inputs to use defaults"
        )
    ...
    try:
        ...
        if default_files:
            raise FileNotFoundError(
                "Could not find default voltage file",
                volt_fname,
                "in current directory.",
            )
        else:
            raise FileNotFoundError(
                "Could not find input voltage file", volt_fname, "."
            )
    ...
    if len(neurons) > 2:
        print(
            "The extra template plot shows additional units that are not a best match to any ground truth unit"
        )
        fig, axes = plt.subplots(1)
    ...
    print(
        "Figure 2 shows the CCG of the sorted units (black) and the ground truth (red)"
    )
    fig, axes = plt.subplots(1)
    counts, time_axis = fbp.analyze_spike_timing.zero_symmetric_ccg(
        neurons[test_match_to_neurons[0]]["spike_indices"],
        neurons[test_match_to_neurons[1]]["spike_indices"],
        20 * 40,
        40,
    )
    axes.bar(time_axis, counts, width=1, color=[0.5, 0.5, 0.5])
    axes.plot(time_axis, counts, color="k", label="Sorted units")

    # CCG for actual data for comparison
    counts, time_axis = fbp.analyze_spike_timing.zero_symmetric_ccg(
        ground_truth[0], ground_truth[1], 20 * 40, 40
    )
    axes.plot(time_axis, counts, color="r", label="Ground truth")

    axes.axvline(0, color=[0.75, 0.75, 0.75])
    axes.axvline(10, color=[0.75, 0.75, 0.75])
    axes.axvline(-10, color=[0.75, 0.75, 0.75])
    axes.axvline(5, color=[0.75, 0.75, 0.75])
    axes.axvline(-5, color=[0.75, 0.75, 0.75])
