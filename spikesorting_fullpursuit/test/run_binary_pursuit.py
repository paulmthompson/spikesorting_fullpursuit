...
dict_other_templates = {
    "upsidedown_47a": np.array(
        [
            -0.1513909,
            -0.16631116,
            -0.17847979,
...
class RunBinaryPursuit(object):
    ...
    def __init__(
        ...
        num_channels,
        duration,
        neuron_templates=None,
        frequency_range=(500, 8000),
        samples_per_second=40000,
        amplitude=1,
        percent_shared_noise=0,
        correlate1_2=False,
        voltage_dtype=np.float32,
    ):
        ...
        self.neuron_templates = (self.neuron_templates * self.amplitude).astype(
            self.voltage_dtype
        )
        self.actual_IDs = []
        self.percent_shared_noise = percent_shared_noise
        if self.percent_shared_noise > 1:
            self.percent_shared_noise = 1.0
        if self.percent_shared_noise < 0.0:
            self.percent_shared_noise = 0.0
    ...
    def gen_poisson_spiketrain(
        ...
        for i in range(
            int(5 * self.samples_per_second / 1000),
            spiketrain.size - int(5 * self.samples_per_second / 1000),
        ):
    ...
    def gen_bandlimited_noise(self):
        ...
        freqs = np.abs(
            np.fft.fftfreq(
                self.samples_per_second * self.duration, 1 / self.samples_per_second
            )
        )
        f = np.zeros(self.samples_per_second * self.duration)
        idx = np.where(
            np.logical_and(
                freqs >= self.frequency_range[0], freqs <= self.frequency_range[1]
            )
        )[0]
        f[idx] = 1
        ...
        bandlimited_noise = (
            bandlimited_noise * self.amplitude / (3 * np.std(bandlimited_noise))
        )
        bandlimited_noise = bandlimited_noise.astype(self.voltage_dtype)
        return bandlimited_noise

    def gen_noise_voltage_array(self):
        self.voltage_array = np.zeros(
            (self.num_channels, int(self.samples_per_second * self.duration)),
            dtype=self.voltage_dtype,
        )
        ...
        for i in range(0, self.num_channels):
            self.voltage_array[i, :] += (
                (1 - self.percent_shared_noise) * self.gen_bandlimited_noise()
            ).astype(self.voltage_dtype)
    ...
    def gen_test_dataset(
        ...
        scaling.
        """
        if len(firing_rates) != len(template_inds) or len(firing_rates) != len(
            chan_scaling_factors
        ):
            raise ValueError(
                "Input rates, templates, and scaling factors must all be the same length!"
            )
        ...
        self.actual_templates = [
            np.zeros((1, 0), dtype=self.voltage_dtype)
            for neur in range(0, len(firing_rates))
        ]
        ...
        for neuron in range(0, len(firing_rates)):
            ...
            spiketrain = self.gen_poisson_spiketrain(
                firing_rate=firing_rates[neuron],
                tau_ref=refractory_wins[neuron],
            )
        ...
        if remove_overlaps:
            ...
            for n1 in range(0, len(self.actual_IDs)):
                overlapping_bools.append(
                    np.ones(self.actual_IDs[n1].shape[0], dtype="bool")
                )
                for n2 in range(0, len(self.actual_IDs)):
                    ...
                    overlapping_spike_bool = find_overlapping_spike_bool(
                        self.actual_IDs[n1],
                        self.actual_IDs[n2],
                        overlap_tol=self.neuron_templates[0].shape[0],
                    )
                    overlapping_bools[n1] = np.logical_and(
                        overlapping_bools[n1], ~overlapping_spike_bool
                    )
        ...
        for neuron in range(0, len(firing_rates)):
            ...
            if self.correlate1_2 and neuron == 1:
                ...
                select_inds0 = np.random.choice(
                    self.actual_IDs[neuron - 1].shape[0],
                    n_correlated_spikes,
                    replace=False,
                )
                select_inds1 = np.random.choice(
                    self.actual_IDs[neuron].shape[0], n_correlated_spikes, replace=False
                )
                self.actual_IDs[neuron][select_inds1] = self.actual_IDs[neuron - 1][
                    select_inds0
                ] + np.random.randint(0, 10, n_correlated_spikes)
                self.actual_IDs[neuron].sort()
                overlapping_spike_bool = find_overlapping_spike_bool(
                    self.actual_IDs[neuron],
                    self.actual_IDs[neuron],
                    overlap_tol=int(1.5e-3 * 40000),
                    except_equal=True,
                )
                self.actual_IDs[neuron] = self.actual_IDs[neuron][
            ...
            for chan in range(0, self.num_channels):
                ...
                convolve_kernel = (
                    chan_scaling_factors[neuron][chan]
                    * self.neuron_templates[template_inds[neuron], :]
                )
                # Filter the kernel to match the noise band
                convolve_kernel = signal.filtfilt(
                    b_filt, a_filt, convolve_kernel, axis=0, padlen=None
                )
                convolve_kernel = convolve_kernel.astype(self.voltage_dtype)
                self.actual_templates[neuron] = np.hstack(
                    (self.actual_templates[neuron], convolve_kernel.reshape(1, -1))
                )
                if chan_scaling_factors[neuron][chan] <= 0:
                    continue
                self.voltage_array[chan, :] += (
                    signal.fftconvolve(spiketrain, convolve_kernel, mode="same")
                ).astype(self.voltage_dtype)

    def gen_test_dataset_from_spikes(
        self,
        spiketrain,
        template_inds,
        chan_scaling_factors,
        ...
        for neuron in range(0, len(template_inds)):
            ...
            for chan in range(0, self.num_channels):
                ...
                if chan_scaling_factors[neuron][chan] <= 0:
                    continue
                self.voltage_array[chan, :] += (
                    signal.fftconvolve(spiketrain[neuron], convolve_kernel, mode="same")
                ).astype(self.voltage_dtype)

    def sort_test_dataset(
        self,
        add_templates=None,
        sort_templates=None,
        rand_state=None,
        ...
        post_time = (
            self.neuron_templates.shape[1] - (self.neuron_templates.shape[1] // 2)
        ) / self.samples_per_second
        # These sort info values must match voltage and templates and cannot be
        # changed by input
        self.sort_info.update(
            {
        ...
        separability_metrics = neuron_separability.set_bp_threshold(
            self.separability_metrics
        )
        if self.separability_metrics["templates"].size == 0:
            print("All templates were removed")
            self.binary_pursuit_results = {"spike_indices": [], "neuron_labels": []}
            return
        crossings, neuron_labels, bp_bool, _ = binary_pursuit_parallel.binary_pursuit(
            ...
            kernels_path=None,
            max_gpu_memory=self.sort_info["max_gpu_memory"],
        )

        self.binary_pursuit_results = {"spike_indices": [], "label": []}
