...
def zero_symmetric_ccg(
    ...
    samples_axis = np.arange(
        -1 * samples_window, samples_window + d_samples, d_samples
    ).astype(np.int64)
    counts = np.zeros(samples_axis.size, dtype=np.int64)
    samples_axis = (np.floor((samples_axis + d_samples / 2) / d_samples)).astype(
        np.int64
    )
    if spikes_1.size == 0 or spikes_2.size == 0:
        ...
        if return_trains:
            return (
                np.zeros(samples_axis.shape[0], dtype=np.int64),
                samples_axis,
                None,
                None,
            )
    ...
    train_len = (
        int(max(spikes_1[-1], spikes_2[-1])) + 1
    )  # This is samples, so need to add 1
    ...
    for lag_ind, lag in enumerate(samples_axis):
        ...
        # zero point
        if lag > 0:
            counts[lag_ind] = np.count_nonzero(
                np.logical_and(spike_trains_1[0 : -1 * lag], spike_trains_2[lag:])
            )
        elif lag < 0:
            counts[lag_ind] = np.count_nonzero(
                np.logical_and(spike_trains_2[0:lag], spike_trains_1[-1 * lag :])
            )
        else:
            counts[lag_ind] = np.count_nonzero(
                np.logical_and(spike_trains_1, spike_trains_2)
            )
...
def find_overlapping_spike_bool(
    spikes_1,
    spikes_2,
    overlap_tol,
    except_equal=False,
    ...
    while (ind1 < spikes_1.shape[0]) and (ind2 < spikes_2.shape[0]):
        if except_equal and spikes_1[ind1] == spikes_2[ind2]:
            ...
            while (ind1 < spikes_1.shape[0]) and spikes_1[ind1] <= spikes_2[
                ind2
            ] + overlap_tol:
...
def keep_binary_pursuit_duplicates(
    event_indices,
    binary_pursuit_bool,
    tol_inds,
):
...
def remove_binary_pursuit_duplicates(
    event_indices,
    clips,
    unit_template,
    binary_pursuit_bool,
...
def remove_spike_event_duplicates(
    event_indices,
    clips,
    unit_template,
    tol_inds,
...
def remove_spike_event_duplicates_across_chans(combined_neuron):
    ...
    while next_index < event_indices.size:
        if (
            event_indices[next_index] - event_indices[curr_index]
            ...
            temp_sse[0] = np.sum(
                (
                    combined_neuron["clips"][curr_index, :]
                    - combined_neuron["template"][curr_chan]
                )
                ** 2
            )
            temp_sse[1] = np.sum(
                (
                    combined_neuron["clips"][next_index, :]
                    - combined_neuron["template"][next_chan]
                )
                ** 2
            )
            if temp_sse[0] <= temp_sse[1]:
...
def compute_spike_trains(
    ...
    train_len = int(
        np.ceil(
            (min_max_indices[1] - min_max_indices[0] + bin_width_samples / 2)
            / bin_width_samples
        )
        + 1
    )  # Add one because it's a time/samples slice

    # Select spikes in desired index range
    select = np.logical_and(
        spike_indices >= min_max_indices[0], spike_indices <= min_max_indices[1]
    )
    # Convert the spike indices to units of bin_width_samples
    spikes = (
        np.floor(
            (spike_indices[select] + bin_width_samples / 2 - min_max_indices[0])
            / bin_width_samples
        )
    ).astype(np.int64)
...
def calc_overlap_ratio(
    ...
    overlap_win = [max(n1_spikes[0], n2_spikes[0]), min(n1_spikes[-1], n2_spikes[-1])]
    n1_start = next(
        (idx[0] for idx, val in np.ndenumerate(n1_spikes) if val >= overlap_win[0]),
        None,
    )
    if n1_start is None:
        ...
        return 0.0
    n2_start = next(
        (idx[0] for idx, val in np.ndenumerate(n2_spikes) if val >= overlap_win[0]),
        None,
    )
    if n2_start is None:
    ...
    n1_stop = next(
        (idx[0] for idx, val in np.ndenumerate(n1_spikes) if val > overlap_win[1]), None
    )
    n2_stop = next(
        (idx[0] for idx, val in np.ndenumerate(n2_spikes) if val > overlap_win[1]), None
    )

    n1_spike_train = compute_spike_trains(
        n1_spikes[n1_start:n1_stop], max_samples, overlap_win
    )
    n2_spike_train = compute_spike_trains(
        n2_spikes[n2_start:n2_stop], max_samples, overlap_win
    )
    ...
    overlap_ratio = max(
        num_hits / (num_hits + n1_misses), num_hits / (num_hits + n2_misses)
    )
    return overlap_ratio


def calc_expected_overlap(
    spike_inds_1,
    spike_inds_2,
    overlap_time,
    sampling_rate,
    ...
    n1_start = next(
        (idx[0] for idx, val in np.ndenumerate(spike_inds_1) if val >= first_index),
        None,
    )
    n1_stop = next(
        (
            idx[0]
            for idx, val in np.ndenumerate(spike_inds_1[::-1])
            if val <= last_index
        ),
        None,
    )
    ...
    n2_start = next(
        (idx[0] for idx, val in np.ndenumerate(spike_inds_2) if val >= first_index),
        None,
    )
    n2_stop = next(
        (
            idx[0]
            for idx, val in np.ndenumerate(spike_inds_2[::-1])
            if val <= last_index
        ),
        None,
    )
...
def calc_expected_overlap_ratio(
    spike_inds_1,
    spike_inds_2,
    overlap_time,
    sampling_rate,
    ...
    As usual, spike_indices within each neuron must be sorted."""
    expected_overlap, n1_count, n2_count = calc_expected_overlap(
        spike_inds_1, spike_inds_2, overlap_time, sampling_rate
    )
...
def calc_ccg_overlap_ratios(
    ...
    counts, x_vals = zero_symmetric_ccg(
        spike_inds_1, spike_inds_2, samples_window, bin_samples
    )
...
def calc_isi_violation_rate(
    ...
    if absolute_refractory_period - duplicate_time <= 0:
        print(
            "LINE 583: duplicate_tol_inds encompasses absolute_refractory_period so fraction MUA cannot be computed."
        )
        return np.nan
    num_isi_violations = np.count_nonzero(
        all_isis / sampling_rate < absolute_refractory_period
    )
...
def mean_firing_rate(spike_indices, sampling_rate):
    ...
    if spike_indices[0] == spike_indices[-1]:
        return 0.0  # Only one spike (could be repeated)
    mean_rate = (
        sampling_rate * spike_indices.size / (spike_indices[-1] - spike_indices[0])
    )
    return mean_rate


def calc_fraction_mua(
    spike_indices,
    sampling_rate,
    duplicate_tol_inds,
    absolute_refractory_period,
    ...
    isi_violation_rate = calc_isi_violation_rate(
        spike_indices, sampling_rate, absolute_refractory_period, duplicate_tol_inds
    )
