...
def align_events_with_template(
    ...
    temp_index[1] = 2 * temp_index[0] + max(int(round(clip_width[1] * sampling_rate)), 1) + 1 # Add one so that last element is included
...
def wavelet_align_events(
    ...
    min_win_freq = 1./((window[1] + window[0])/sampling_rate)
    ...
    if len(temp_scales) == 0:
        ...
        scale = Fc * sampling_rate / (align_band_width[0] + (align_band_width[1] - align_band_width[0]))
        central_template = signal.ricker(2 * center+1, scale)
        temp_scales.append(central_template)
