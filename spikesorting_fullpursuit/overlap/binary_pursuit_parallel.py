...
def compute_shift_indices(templates, samples_per_chan, n_chans):
    ...
    template_pre_inds = np.zeros(
        templates.shape[0] * templates.shape[0], dtype=np.int32
    )
    template_post_inds = np.zeros(
        templates.shape[0] * templates.shape[0], dtype=np.int32
    )
    ...
    for t in range(0, templates.shape[0]):
        ...
        for chan in range(0, n_chans):
            sum_voltage[chan, samples_per_chan : 2 * samples_per_chan] = templates[
                t, :
            ][chan * samples_per_chan : (chan + 1) * samples_per_chan]
            L_phase += np.convolve(
                sum_voltage[chan, :],
                templates[t, :][
                    chan * samples_per_chan : (chan + 1) * samples_per_chan
    ...
    for f in range(0, templates.shape[0]):
        fixed_t = templates[f, :]
        fixed_t_ss = 0.5 * np.sum(fixed_t**2)
        fixed_ind = ref_inds[f]
        for s in range(f, templates.shape[0]):
            shift_t = templates[s, :]
            shift_t_ss = 0.5 * np.sum(shift_t**2)
            ...
            for shift in range(-max_shift, max_shift + 1):
                ...
                for chan in range(0, n_chans):
                    ...
                    sum_voltage[
                        chan, samples_per_chan : 2 * samples_per_chan
                    ] = fixed_t[chan * samples_per_chan : (chan + 1) * samples_per_chan]
                    if shift < 0:
                        sum_voltage[
                            chan,
                            samples_per_chan + shift : 2 * samples_per_chan + shift,
                        ] += shift_t[
                            chan * samples_per_chan : (chan + 1) * samples_per_chan
                        ]
                    else:
            ...
            if np.count_nonzero(failed_assignments) == 0:
                ...
                template_pre_inds[f * templates.shape[0] + s] = (
                    np.argmax(failed_assignments) - max_shift
                )
                # NOTE: Subtracting 1 will give inclusive indices. Not subtracting 1 gives the slice range.
                template_post_inds[f * templates.shape[0] + s] = (
                    failed_assignments.shape[0]
                    - np.argmax(failed_assignments[-1::-1])
                    - max_shift
                )

                # For the same pair in opposite order, these indices are REVERSED
                template_pre_inds[s * templates.shape[0] + f] = (
                    -1 * template_post_inds[f * templates.shape[0] + s] + 1
                )  # Add 1 to undo slice
                template_post_inds[s * templates.shape[0] + f] = (
                    -1 * template_pre_inds[f * templates.shape[0] + s] + 1
                )
...
def binary_pursuit(
    ...
    (
        ...
        sort_info["clip_width"], sampling_rate
    )
    ...
    template_sum_squared_by_channel = np.float32(
        separability_metrics["template_SS_by_chan"].flatten(order="C")
    )
    neuron_lower_thresholds = np.float32(
    ...
    with cl.CommandQueue(ctx) as queue:
        ...
        compute_template_maximum_likelihood_kernel = (
            prg.compute_template_maximum_likelihood
        )
        ...
        resid_local_work_size = np.int64(
            compute_residual_kernel.get_work_group_info(
                cl.kernel_work_group_info.WORK_GROUP_SIZE, device
            )
        )
        likelihood_local_work_size = np.int64(
            compute_full_likelihood_kernel.get_work_group_info(
                cl.kernel_work_group_info.WORK_GROUP_SIZE, device
            )
        )
        max_likelihood_local_work_size = np.int64(
            compute_template_maximum_likelihood_kernel.get_work_group_info(
                cl.kernel_work_group_info.WORK_GROUP_SIZE, device
            )
        )
        overlap_local_work_size = np.int64(
            overlap_recheck_indices_kernel.get_work_group_info(
                cl.kernel_work_group_info.WORK_GROUP_SIZE, device
            )
        )
        parse_overlap_local_work_size = np.int64(
            parse_overlap_recheck_indices_kernel.get_work_group_info(
                cl.kernel_work_group_info.WORK_GROUP_SIZE, device
            )
        )
        check_overlap_local_work_size = np.int64(
            check_overlap_reassignments_kernel.get_work_group_info(
                cl.kernel_work_group_info.WORK_GROUP_SIZE, device
            )
        )
        pursuit_local_work_size = np.int64(
            binary_pursuit_kernel.get_work_group_info(
                cl.kernel_work_group_info.WORK_GROUP_SIZE, device
            )
        )
        adjusted_local_work_size = np.int64(
            get_adjusted_clips_kernel.get_work_group_info(
                cl.kernel_work_group_info.WORK_GROUP_SIZE, device
            )
        )
        ...
        max_enqueue_likelihood = np.int64(
            likelihood_local_work_size * (device.max_compute_units)
        )
        max_enqueue_max_likelihood = np.int64(
            max_likelihood_local_work_size * (device.max_compute_units)
        )
        max_enqueue_overlap = np.int64(
            overlap_local_work_size * (device.max_compute_units)
        )
        max_enqueue_parse_overlap = np.int64(
            parse_overlap_local_work_size * (device.max_compute_units)
        )
        max_enqueue_check_overlap = np.int64(
            check_overlap_local_work_size * (device.max_compute_units)
        )
        max_enqueue_pursuit = np.int64(
            pursuit_local_work_size * (device.max_compute_units)
        )
        max_enqueue_adjusted = np.int64(
            adjusted_local_work_size * (device.max_compute_units)
        )

        # Determine the segment size that we need to use to fit into the
        # memory on this GPU device.
        gpu_memory_size = device.get_info(
        ...
        gpu_memory_size = max(
            gpu_memory_size * 0.5, gpu_memory_size - (1024 * 1024 * 1024)
        )
        ...
        if (sys_platform.system() == "Windows") and (
            float(regex_version.group()) >= 10.0
        ):
            gpu_memory_size = min(
                device.get_info(cl.device_info.GLOBAL_MEM_SIZE) * 0.75, gpu_memory_size
            )
        ...
        # or risk overflow
        items_per_index = np.int64(
            (2 * n_max_shift_inds + 1) * (2 * n_max_shift_inds + 1) * templates.shape[0]
        )
        ...
        memory_usage_per_second = (
            n_chans * sampling_rate * np.dtype(np.float32).itemsize
        )
        # Add usage for likelihood functions
        memory_usage_per_second += (
            templates.shape[0] * sampling_rate * np.dtype(np.float32).itemsize
        )
        ...
        memory_usage_per_second += (sampling_rate / template_samples_per_chan) * (
            ...
            + np.dtype(np.float32).itemsize  # best labels
            +  # best likelihoods
            ...
            # most cases.
            (n_local_ID / 3) * np.dtype(np.float32).itemsize
            + (n_local_ID / 3) * np.dtype(np.uint32).itemsize  # Overlap likelihoods
        )  # Overlap labels
        ...
        max_addressable_seconds = ((1 << device_address_bits) - 1) / (
            np.dtype(np.float32).itemsize * sampling_rate * n_chans
        )
        num_seconds_per_chunk = min(
            num_seconds_per_chunk, np.floor(max_addressable_seconds)
        )
        ...
        if num_indices_per_chunk < 4 * template_samples_per_chan:
            raise ValueError(
                "Cannot fit enough data on GPU to run binary pursuit. Decrease template samples or get a new GPU."
            )
        ...
        template_buffer = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=templates_vector
        )
        template_sum_squared_buffer = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=template_sum_squared
        )
        template_sum_squared_by_channel_buffer = cl.Buffer(
            ctx,
            mf.READ_ONLY | mf.COPY_HOST_PTR,
            hostbuf=template_sum_squared_by_channel,
        )
        ...
        for chunk_number, start_index in enumerate(chunk_onsets):
            ...
            voltage_buffer = cl.Buffer(
                ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=chunk_voltage
            )
            full_likelihood_function_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(
                    chunk_voltage_length * templates.shape[0], dtype=np.float32
                ),
            )

            # Set arguments that are the same every iteration
            compute_residual_kernel.set_arg(
                0, voltage_buffer
            )  # location where chunk voltage is stored
            compute_residual_kernel.set_arg(
                1, chunk_voltage_length
            )  # length of chunk voltage array
            compute_residual_kernel.set_arg(
                2, n_chans
            )  # Number of neighboring channels
            compute_residual_kernel.set_arg(
                3, template_buffer
            )  # location where our templates are stored
            compute_residual_kernel.set_arg(
                4, np.uint32(templates.shape[0])
            )  # Number of neurons (`rows` in templates)
            compute_residual_kernel.set_arg(
                5, np.uint32(template_samples_per_chan)
            )  # Number of timepoints in each channel of templates
            ...
            num_template_widths = int(
                np.ceil(chunk_voltage_length / template_samples_per_chan)
            )
            # These are the work sizes that need computed before while loop below
            # then updated at the end of while loop
            total_work_size_likelihood = likelihood_local_work_size * np.int64(
                np.ceil(
                    (num_template_widths * templates.shape[0])
                    / likelihood_local_work_size
                )
            )
            total_work_size_max_likelihood = max_likelihood_local_work_size * np.int64(
                np.ceil(num_template_widths / max_likelihood_local_work_size)
            )
            total_work_size_pursuit = pursuit_local_work_size * np.int64(
                np.ceil(num_template_widths / pursuit_local_work_size)
            )

            # Construct our buffers
            lower_thresholds_buffer = cl.Buffer(
                ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=neuron_lower_thresholds
            )
            # upper_thresholds_buffer = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=neuron_upper_thresholds)
            num_additional_spikes_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(1, dtype=np.uint32),
            )  # NOTE: Must be :rw for atomic to work
            additional_spike_indices_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(num_template_widths, dtype=np.uint32),
            )
            additional_spike_labels_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(num_template_widths, dtype=np.uint32),
            )
            best_spike_likelihoods_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(num_template_widths, dtype=np.float32),
            )
            best_spike_labels_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(num_template_widths, dtype=np.uint32),
            )
            best_spike_indices_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(num_template_widths, dtype=np.uint32),
            )
            window_indices_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.arange(0, num_template_widths, 1, dtype=np.uint32),
            )
            next_check_window_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(num_template_widths, dtype=np.uint8),
            )
            overlap_recheck_window_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(num_template_widths, dtype=np.uint8),
            )
            overlap_window_indices_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.arange(0, num_template_widths, 1, dtype=np.uint32),
            )
            overlap_best_spike_indices_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(num_template_widths, dtype=np.uint32),
            )
            overlap_best_spike_labels_buffer = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=np.zeros(num_template_widths, dtype=np.uint32),
            )
            ...
            compute_full_likelihood_kernel.set_arg(
                0, voltage_buffer
            )  # Voltage buffer created previously
            compute_full_likelihood_kernel.set_arg(
                1, chunk_voltage_length
            )  # Length of chunk voltage
            compute_full_likelihood_kernel.set_arg(
                2, n_chans
            )  # number of neighboring channels
            compute_full_likelihood_kernel.set_arg(
                3, template_buffer
            )  # Our template buffer, created previously
            compute_full_likelihood_kernel.set_arg(
                4, np.uint32(templates.shape[0])
            )  # Number of unique neurons, M
            compute_full_likelihood_kernel.set_arg(
                5, np.uint32(template_samples_per_chan)
            )  # Number of timepoints in each template
            compute_full_likelihood_kernel.set_arg(
                6, template_sum_squared_buffer
            )  # Sum of squared templated
            compute_full_likelihood_kernel.set_arg(
                7, window_indices_buffer
            )  # Actual window indices to check
            compute_full_likelihood_kernel.set_arg(
                8, np.uint32(num_template_widths)
            )  # Number of actual window indices to check
            compute_full_likelihood_kernel.set_arg(9, full_likelihood_function_buffer)

            # Set input arguments for template maximum likelihood kernel
            compute_template_maximum_likelihood_kernel.set_arg(
                0, chunk_voltage_length
            )  # Length of chunk voltage
            compute_template_maximum_likelihood_kernel.set_arg(
                1, np.uint32(template_samples_per_chan)
            )  # Number of timepoints in each template
            compute_template_maximum_likelihood_kernel.set_arg(
                2, np.uint32(0)
            )  # Template number
            compute_template_maximum_likelihood_kernel.set_arg(
                3, window_indices_buffer
            )  # Actual window indices to check
            compute_template_maximum_likelihood_kernel.set_arg(
                4, np.uint32(num_template_widths)
            )  # Number of actual window indices to check
            compute_template_maximum_likelihood_kernel.set_arg(
                5, best_spike_indices_buffer
            )  # Storage for peak likelihood index
            compute_template_maximum_likelihood_kernel.set_arg(
                6, best_spike_labels_buffer
            )  # Storage for peak likelihood label
            compute_template_maximum_likelihood_kernel.set_arg(
                7, best_spike_likelihoods_buffer
            )  # Storage for peak likelihood value
            compute_template_maximum_likelihood_kernel.set_arg(
                8, next_check_window_buffer
            )  # Binary vector indicating whether a window at its index needs checked on next iteration of binary_pursuit kernel
            compute_template_maximum_likelihood_kernel.set_arg(
                9, overlap_recheck_window_buffer
            )
            compute_template_maximum_likelihood_kernel.set_arg(
                10, overlap_best_spike_indices_buffer
            )  # Storage for new best overlap indices
            compute_template_maximum_likelihood_kernel.set_arg(
                11, overlap_best_spike_labels_buffer
            )  # Storage for new best overlap indices
            compute_template_maximum_likelihood_kernel.set_arg(
                12, full_likelihood_function_buffer
            )
            compute_template_maximum_likelihood_kernel.set_arg(
                13, lower_thresholds_buffer
            )

            # Set input arguments for overlap recheck kernel
            overlap_recheck_indices_kernel.set_arg(
                0, voltage_buffer
            )  # Voltage buffer created previously
            overlap_recheck_indices_kernel.set_arg(
                1, chunk_voltage_length
            ...
            overlap_recheck_indices_kernel.set_arg(
                6, overlap_window_indices_buffer
            )  # Actual window indices to check
            overlap_recheck_indices_kernel.set_arg(
                7, np.uint32(num_template_widths)
            )  # Number of actual window indices to check
            overlap_recheck_indices_kernel.set_arg(
                8, best_spike_indices_buffer
            )  # Storage for peak likelihood index
            overlap_recheck_indices_kernel.set_arg(
                9, best_spike_labels_buffer
            )  # Storage for peak likelihood label
            ...
            overlap_local_likelihood_buffer = cl.LocalMemory(
                4 * overlap_local_work_size
            )
            ...
            overlap_recheck_indices_kernel.set_arg(16, lower_thresholds_buffer)

            # Set input arguments for parse overlap recheck kernel
            parse_overlap_recheck_indices_kernel.set_arg(
                0, chunk_voltage_length
            )  # Length of chunk voltage
            parse_overlap_recheck_indices_kernel.set_arg(
                1, np.uint32(templates.shape[0])
            )  # Number of unique neurons, M
            parse_overlap_recheck_indices_kernel.set_arg(
                2, overlap_window_indices_buffer
            )  # Actual window indices to check
            parse_overlap_recheck_indices_kernel.set_arg(
                3, np.uint32(num_template_widths)
            )  # Number of actual window indices to check
            parse_overlap_recheck_indices_kernel.set_arg(
                4, best_spike_indices_buffer
            )  # Storage for peak likelihood index
            parse_overlap_recheck_indices_kernel.set_arg(
                5, best_spike_labels_buffer
            )  # Storage for peak likelihood label
            parse_overlap_recheck_indices_kernel.set_arg(
                6, best_spike_likelihoods_buffer
            )  # Storage for peak likelihood value
            parse_overlap_recheck_indices_kernel.set_arg(
                7, overlap_best_spike_indices_buffer
            )  # Storage for new best overlap indices
            parse_overlap_recheck_indices_kernel.set_arg(
                8, overlap_best_spike_labels_buffer
            )  # Storage for new best overlap indices
            parse_overlap_recheck_indices_kernel.set_arg(
                9, full_likelihood_function_buffer
            )
            ...
            parse_overlap_recheck_indices_kernel.set_arg(
                14, overlap_recheck_window_buffer
            )

            check_overlap_reassignments_kernel.set_arg(
                0, chunk_voltage_length
            )  # Length of chunk voltage
            check_overlap_reassignments_kernel.set_arg(
                1, np.uint32(template_samples_per_chan)
            ...
            binary_pursuit_kernel.set_arg(
                0, chunk_voltage_length
            )  # Length of chunk voltage
            binary_pursuit_kernel.set_arg(1, n_chans)  # number of neighboring channels
            binary_pursuit_kernel.set_arg(
                2, np.uint32(template_samples_per_chan)
            )  # Number of timepoints in each template
            binary_pursuit_kernel.set_arg(3, lower_thresholds_buffer)  # Bias
            binary_pursuit_kernel.set_arg(
                4, window_indices_buffer
            )  # Actual window indices to check
            binary_pursuit_kernel.set_arg(
                5, np.uint32(num_template_widths)
            )  # Number of actual window indices to check
            binary_pursuit_kernel.set_arg(
                6, best_spike_indices_buffer
            )  # Storage for peak likelihood index
            binary_pursuit_kernel.set_arg(
                7, best_spike_labels_buffer
            )  # Storage for peak likelihood label
            binary_pursuit_kernel.set_arg(
                8, best_spike_likelihoods_buffer
            )  # Storage for peak likelihood value
            binary_pursuit_kernel.set_arg(9, local_buffer)  # Local buffer
            binary_pursuit_kernel.set_arg(
                10, num_additional_spikes_buffer
            )  # Output, total number of additional spikes
            binary_pursuit_kernel.set_arg(
                11, additional_spike_indices_buffer
            )  # Additional spike indices
            binary_pursuit_kernel.set_arg(
                12, additional_spike_labels_buffer
            )  # Additional spike labels
            binary_pursuit_kernel.set_arg(13, overlap_recheck_window_buffer)
            binary_pursuit_kernel.set_arg(
                14, next_check_window_buffer
            )  # Binary vector indicating whether a window at its index needs checked on next iteration of binary_pursuit kernel
            ...
            n_loops = 0
            print(
                "pursuit total size is",
                total_work_size_pursuit,
                "local size is",
                pursuit_local_work_size,
                "with max enqueue",
                max_enqueue_pursuit,
                "chose n to enqueue",
                min(total_work_size_pursuit, max_enqueue_pursuit),
            )
            print(
                "overlaps local size is",
                overlap_local_work_size,
                "with max enqueue",
                max_enqueue_overlap,
            )
            ...
            while True:
                ...
                for template_index in range(0, templates.shape[0]):
                    ...
                    compute_template_maximum_likelihood_kernel.set_arg(
                        2, np.uint32(template_index)
                    )  # Template number
                    for enqueue_step in np.arange(
                        np.int64(0),
                        total_work_size_max_likelihood,
                        n_to_enqueue,
                        dtype=np.int64,
                    ):
                        temp_ml_event = cl.enqueue_nd_range_kernel(
                            ...
                            compute_template_maximum_likelihood_kernel,
                            (n_to_enqueue,),
                            (max_likelihood_local_work_size,),
                            global_work_offset=(enqueue_step,),
                            wait_for=next_wait_event,
                        )
                        queue.finish()
                ...
                next_wait_event = [
                    cl.enqueue_copy(
                        queue,
                        overlap_recheck_window,
                        overlap_recheck_window_buffer,
                        wait_for=next_wait_event,
                    )
                ]
                ...
                overlap_window_indices = np.uint32(
                    np.nonzero(overlap_recheck_window)[0]
                )
                if (
                    overlap_window_indices.shape[0] > 0
                    and sort_info["do_overlap_recheck"]
                ):
                    ...
                    n_local_ID_leftover = np.int64(
                        (overlap_local_work_size * n_local_ID) % items_per_index
                    )
                    # Total work is then indices, times number per index, plus the leftovers from work groups
                    total_work_size_overlap = np.int64(
                        overlap_window_indices.shape[0] * items_per_index
                        + overlap_window_indices.shape[0] * n_local_ID_leftover
                    )
                    # Round to nearest work group size
                    total_work_size_overlap = overlap_local_work_size * np.int64(
                        np.ceil(total_work_size_overlap / overlap_local_work_size)
                    )
                    ...
                    if total_work_size_overlap > (1 << device_address_bits) - 1:
                        ...
                        print(
                            "Current work size of",
                            total_work_size_overlap,
                            "will OVERFLOW a",
                            device_address_bits,
                            "bit graphics card.",
                        )
                        print(
                            "Decrease segment duration/number of channels in data file or get a new GPU."
                        )
                    ...
                    enqueue_by_int32 = np.int64(
                        overlap_local_work_size
                        * np.ceil(
                            (2**32 - overlap_local_work_size)
                            / overlap_local_work_size
                        )
                    )
                    ...
                    overlap_recheck_indices_kernel.set_arg(
                        7, np.uint32(overlap_window_indices.shape[0])
                    )  # Number of actual window indices to check
                    parse_overlap_recheck_indices_kernel.set_arg(
                        3, np.uint32(overlap_window_indices.shape[0])
                    )
                    num_work_groups = overlap_window_indices.shape[0] * n_local_ID

                    if n_loops == 1:
                        overlap_group_best_likelihood_buffer = cl.Buffer(
                            ctx,
                            mf.READ_WRITE | mf.COPY_HOST_PTR,
                            hostbuf=np.zeros(num_work_groups, dtype=np.float32),
                        )
                        overlap_recheck_indices_kernel.set_arg(
                            14, overlap_group_best_likelihood_buffer
                        )
                        parse_overlap_recheck_indices_kernel.set_arg(
                            12, overlap_group_best_likelihood_buffer
                        )
                        overlap_group_best_work_id_buffer = cl.Buffer(
                            ctx,
                            mf.READ_WRITE | mf.COPY_HOST_PTR,
                            hostbuf=np.zeros(num_work_groups, dtype=np.uint32),
                        )
                        overlap_recheck_indices_kernel.set_arg(
                            15, overlap_group_best_work_id_buffer
                        )
                        parse_overlap_recheck_indices_kernel.set_arg(
                            13, overlap_group_best_work_id_buffer
                        )
                        curr_overlaps_buffer_size = num_work_groups
                        print(
                            "Made buffers for a total of",
                            num_work_groups,
                            "work groups",
                        )
                    ...
                    for enqueue_step in np.arange(
                        ...
                        if enqueue_step + n_to_enqueue > total_work_size_overlap:
                            n_to_enqueue = overlap_local_work_size * np.int64(
                                np.ceil(
                                    (total_work_size_overlap - enqueue_step)
                                    / overlap_local_work_size
                                )
                            )
                        overlap_event = cl.enqueue_nd_range_kernel(
                            queue,
                    ...
                    total_work_size_parse_overlap = (
                        parse_overlap_local_work_size
                        * np.int64(
                            np.ceil(
                                overlap_window_indices.shape[0]
                                / parse_overlap_local_work_size
                            )
                        )
                    )
                    ...
                    total_work_size_check_overlap = (
                        check_overlap_local_work_size
                        * np.int64(
                            np.ceil(
                                overlap_window_indices.shape[0]
                                / check_overlap_local_work_size
                ...
                for enqueue_step in np.arange(
                    np.int64(0), total_work_size_pursuit, n_to_enqueue, dtype=np.int64
                ):
                    pursuit_event = cl.enqueue_nd_range_kernel(
                        queue,
                ...
                cl.enqueue_copy(
                    queue,
                    num_additional_spikes,
                    num_additional_spikes_buffer,
                    wait_for=next_wait_event,
                )
                ...
                compute_full_likelihood_kernel.set_arg(
                    8, np.uint32(new_window_indices.shape[0])
                )  # Number of actual window indices to check
                compute_template_maximum_likelihood_kernel.set_arg(
                    4, np.uint32(new_window_indices.shape[0])
                )  # Number of actual window indices to check
                binary_pursuit_kernel.set_arg(
                    5, np.uint32(new_window_indices.shape[0])
                )  # Number of actual window indices to check
                # Reset number of kernels to run for next pass
                total_work_size_pursuit = pursuit_local_work_size * np.int64(
                    np.ceil(new_window_indices.shape[0] / pursuit_local_work_size)
                )
                total_work_size_likelihood = likelihood_local_work_size * np.int64(
                    np.ceil(
                        (new_window_indices.shape[0] * templates.shape[0])
                        / likelihood_local_work_size
                    )
                )
                total_work_size_max_likelihood = (
                    ...
                    * np.int64(
                        np.ceil(
                            new_window_indices.shape[0] / max_likelihood_local_work_size
                        )
                    )
                )
                ...
                additional_spike_indices = np.zeros(
                    num_additional_spikes[0], dtype=np.uint32
                )
                additional_spike_labels = np.zeros(
                    num_additional_spikes[0], dtype=np.uint32
                )
                next_wait_event = []
                # Already waited for pursuit to finish above
                next_wait_event.append(
                    cl.enqueue_copy(
                        queue,
                        additional_spike_indices,
                        additional_spike_indices_buffer,
                        wait_for=None,
                    )
                )
                next_wait_event.append(
                    cl.enqueue_copy(
                        queue,
                        additional_spike_labels,
                        additional_spike_labels_buffer,
                        wait_for=None,
                    )
                )
                ...
                total_work_size_resid = resid_local_work_size * np.int64(
                    np.ceil((num_additional_spikes[0]) / resid_local_work_size)
                )

                n_to_enqueue = np.amin([total_work_size_resid, max_enqueue_resid])
                for enqueue_step in np.arange(
                    np.int64(0), total_work_size_resid, n_to_enqueue, dtype=np.int64
                ):
                    residual_event = cl.enqueue_nd_range_kernel(
                        queue,
                        compute_residual_kernel,
                        (n_to_enqueue,),
                        (resid_local_work_size,),
                        global_work_offset=(enqueue_step,),
                        wait_for=next_wait_event,
                    )
                    queue.finish()
            ...
            if (chunk_total_additional_spikes + chunk_crossings.shape[0]) > 0:
                if chunk_total_additional_spikes == 0:
                    ...
                    all_chunk_crossings = np.hstack(
                        (
                            np.hstack(chunk_total_additional_spike_indices),
                            chunk_crossings,
                        )
                    )
                    all_chunk_labels = np.hstack(
                        (np.hstack(chunk_total_additional_spike_labels), chunk_labels)
                    )

                print(
                    "Found",
                    chunk_total_additional_spikes,
                    "secret spikes this chunk.",
                    flush=True,
                )

                if sort_info["get_adjusted_clips"]:
                    print(
                        "Getting adjusted clips for",
                        all_chunk_crossings.shape[0],
                        "total spikes this chunk",
                        flush=True,
                    )

                    all_adjusted_clips = np.zeros(
                        (chunk_total_additional_spikes + chunk_crossings.shape[0])
                        * templates.shape[1],
                        dtype=np.float32,
                    )
                    all_chunk_crossings_buffer = cl.Buffer(
                        ctx,
                        mf.READ_ONLY | mf.COPY_HOST_PTR,
                        hostbuf=all_chunk_crossings,
                    )
                    all_chunk_labels_buffer = cl.Buffer(
                        ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=all_chunk_labels
                    )
                    all_adjusted_clips_buffer = cl.Buffer(
                        ctx,
                        mf.WRITE_ONLY | mf.COPY_HOST_PTR,
                        hostbuf=all_adjusted_clips,
                    )
                    ...
                    get_adjusted_clips_kernel.set_arg(
                        5, np.uint32(template_samples_per_chan)
                    )
                    ...
                    cl.enqueue_copy(
                        queue,
                        all_adjusted_clips,
                        all_adjusted_clips_buffer,
                        wait_for=clip_events,
                    )
                    queue.finish()
                    all_chunk_crossings_buffer.release()
                    all_chunk_labels_buffer.release()
                    all_adjusted_clips_buffer.release()
                    all_adjusted_clips = np.reshape(
                        all_adjusted_clips,
                        (all_chunk_crossings.shape[0], templates.shape[1]),
                    )
                else:
                    all_adjusted_clips = []
    ...
    if len(secret_spike_indices) > 0:
        ...
        print(
            "Found a total of",
            np.count_nonzero(binary_pursuit_spike_bool),
            "secret spikes",
            flush=True,
        )
    else:
